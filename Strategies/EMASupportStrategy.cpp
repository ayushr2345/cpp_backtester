#include "EMASupportStrategy.h"
#include <iostream>

namespace strategies
{
    EMASupportStrategy::EMASupportStrategy(const int period)
        : m_ema(period)
    {
    }
    ...
    std::vector<ePosition> EMASupportStrategy::GetExecutionArray(const StockQuote& stockQuote)
    {
        ...
        std::vector<double> movingAverage = m_ema.CalculateMovingAverage(stockQuote);
        for (auto ma : movingAverage)
        {
            std::cout << ma << " ";
        }
        std::cout << std::endl;
